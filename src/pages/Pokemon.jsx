...
function Pokemon(){
    ...
    return (
        <Wrapper>
            ...
            <Link className={classes['link']} to = '/'>Explore more Pokémon</Link>
            </div>
            <h1 className={classes['headline']}>{pokemon.name} {pokemonNumber(pokemon.id)}</h1>
            <div className={classes['pokemon-page']}>
                <div>
                <div className={classes['pokemon-page-img']}>
                    <img width="90%" height="90%" src={pokemon.sprites.other["official-artwork"]["front_default"]} alt="img" />
                </div>
                </div>
                <div className={classes['statistics']}>
                    ...
                    <div>
                        <h3>Abilities</h3>
                        <p>{!pokemon.abilities[0]?.is_hidden ? pokemon.abilities[0]?.ability.name: null}</p>
                        <p>{!pokemon.abilities[1]?.is_hidden ? pokemon.abilities[1]?.ability.name: null}</p>
                    </div>
                </div>
                
                <div className={classes['stats']}>
                    <h2>Stats</h2>
                    {pokemon.stats.map(el => stateRes(el)).map((el,i) => {
                    return (
