...
function MainMenu({ countOnPageRes, typedData, isLoaded }) {
  ...
  if (pagesCount < 7) {
    for (let i = firstvalue; i <= pagesCount; i++) {
      pageItems.push(
        <button
          className={classes["boo-element"]}
          onClick={() => setCurrentPage(i)}
          style={
            currentPage === i
              ? { color: "white", backgroundColor: "#397f84" }
              : {}
          }
          key={uuid()}
        >
      ...
      {isLoaded ? (
        <div className={classes["main-menu"]}>
          {!typedData.length ? (
            ...
            typedData.map((el, i) => {
              if (
                i >= (currentPage - 1) * countOnPageRes &&
                i < countOnPageRes * currentPage
              ) {
                const str = el.name;
                ...
                return (
                  <div key={uuid()} className={classes["item"]}>
                    <div
                      className={classes["item-img-div"]}
                    ...
                    <div className={classes["item-name"]} key={i + 1001}>
                      {str2}
                    </div>
                    <div key={uuid()}>{pokemonNumber(el.id)}</div>
                    <div key={uuid()}>
                      {el.types[0].type.name}
                      {el.types[1]?.type.name && ", " + el.types[1]?.type.name}
                    </div>
                  </div>
                );
              }
          ...
          onClick={() =>
            setCurrentPage((prev) => {
              if (currentPage < pagesCount) {
                setLastValue((prev) =>
                  prev + 1 < currentPage ? prev + 1 : prev
                );
